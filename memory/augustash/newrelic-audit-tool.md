...
`templates/newrelic/nr-pull.sh` pulls a worker-saturation / performance exhibit for a Pantheon site over NerdGraph (NRQL via GraphQL POST to `api.newrelic.com/graphql`, header `API-Key: NRAK-…`). `templates/newrelic/nr-report.py` then turns that JSON into a self-contained `report.html` (inline SVG charts, no internet) + CSVs + an auto-computed per-period **median/worst-day** stats table; optional per-site `breakpoints.csv` (date,label) draws fix markers and splits the periods, and optional `intro.html` injects your narrative (the tool never auto-writes prose — charts are mechanical, the story is yours). Per-site creds (account id, User key, app name) go in an `nr.env` that **must live outside this repo** (e.g. `~/.config/newrelic/<site>/`) — the package ships to every project, so a real key here would propagate. Full reference: `templates/newrelic/queries.md` and `README.md`.

**Reporting saturation honestly:** use **median, not mean** — saturation metrics are spike-driven, so a mean is dragged up by a few stampede-days and overstates the typical rate. The clearest single signal is **count of saturating days** (e.g. daily peak > 50s): MSP went 148/148 → 0/10 across two fixes. Cross-check New Relic spike-days against the FPM `max_children` log dates — when both independent sources agree on the same days, the signal is real.
