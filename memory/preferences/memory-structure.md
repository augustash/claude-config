...
- **Reusable debugging approaches** — methodology that applies beyond a single fix, especially across projects (e.g., "how to track down what's causing max-age: 0 in Drupal"). If the approach is project-agnostic, it belongs in the shared package's `memory/` directory (authored in the `augustash/claude-config` repo), not per-project.
