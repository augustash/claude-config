...
- [site-update](skills/site-update/SKILL.md) — a routine dependency-update round on a site (Drupal or WordPress); owns patch triage for every Drupal skill, so also whenever a patch fails to apply or an edited one seems to be ignored, and right after upgrading ddev itself
