...
- [The browser return and the webhook do the same work twice](memory/drupal/commerce-stripe-return-webhook-race.md) — an express order ends on the generic error page but actually placed, or one intent is recorded as two payments while Stripe shows one charge
...
- [The order screen and commerce_payment_method.type disagree about wallets](memory/drupal/commerce-stripe-wallet-not-in-method-type.md) — auditing or reporting Stripe wallet usage programmatically, or a wallet-share query returning zero while the orders show otherwise
...
- [Testing Apple, Google, Amazon Pay and Affirm in sandbox](memory/drupal/stripe-wallet-testing.md) — before hunting a wallet button that never appears, ruling out a browser, or a sandbox that wants a credential you do not have
- [PayPal's sandbox only takes cards it generated](memory/drupal/paypal-sandbox-buyer.md) — a test card number is refused at checkout, or you need to swap sandbox buyers mid-test
- [An abandoned offsite payment leaves the order locked](memory/drupal/offsite-gateway-abandonment-lock.md) — a customer backs out at PayPal or Affirm and returns to an empty cart; also before tidying an order while someone is at a gateway
...
- [mailsystem resolves a per-module sender before its defaults](memory/drupal/mailsystem-per-module-overrides-defaults.md) — one module's mail vanishes with no error while everything else reaches Mailpit
...
- [Registering a theme SDC with Alchemist in code](memory/augustash/neo-alchemist-component-registration.md) — a placed component renders nothing with nothing logged, or a later command dies in EntityBase.php
